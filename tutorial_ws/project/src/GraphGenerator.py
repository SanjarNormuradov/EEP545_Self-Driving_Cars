...
def euclidean_halton_graph(n, radius, bases, lower, upper, source, target, mapFile, car_width, car_length,
    ...
    print '[Graph Generator] Populating node...'
    while numVertices < n:
    ...
    print '[Graph Generator] Populating edges...'
    for i in xrange(n):
        ...
        for j in xrange(indices.shape[0]):
            ...
            if distances[j] > numpy.finfo(float).eps:
                ...
                edgeCount += 1
        print '[Graph Generator] %d of %d nodes complete, edges: %d' %(i, n, edgeCount)

    print '[Graph Generator] Graph generation complete'
    return G
...
def generate_graph_file(map_msg, halton_points, disc_radius, car_width, car_length, collision_delta):
    file_dir = os.path.expanduser('$(find project)/graph_files')
...
if __name__ == "__main__":
    ...
    # Settings
    halton_points = 500
    disc_radius = int(numpy.sqrt(2 * halton_points))  # 2 * halton_points ** (-0.5)
    print "Disc Radius: " + disc_radius

    for i in range(1):
        print i
        # (pseudo-)random number generation, i.e. the same numbers each iteration
        numpy.random.seed(0)
