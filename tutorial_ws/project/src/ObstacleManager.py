import rospy, cv2, numpy, math, Utils, csv
...
class ObstacleManager(object):
	...
	def __init__(self, mapMsg, 
		...
		# print "[Obstacle Manager] Number of elements in binary image:"
		# print mapMsg.info.height * mapMsg.info.width
		# print "[Obstacle Manager] Number of zeros in binary image:"
		# print len(self.mapImageBW[self.mapImageBW == 0])
		# print "[Obstacle Manager] Number of zeros to all element number ratio:"
		# print float(len(self.mapImageBW[self.mapImageBW == 0])) / (mapMsg.info.height * mapMsg.info.width) * 100
		...
		# Add bad waypoints into the map
		badwaypoints = []
		with open(rospy.get_param("~badwaypoints_filename"), "r") as csv_file:
			badwaypoints_file = csv.DictReader(csv_file)
			for crd_dict in badwaypoints_file:
				badwaypoints.append([int(crd_dict['x']), int(crd_dict['y'])])
		for badwaypoint in badwaypoints:
			for pnt_x in xrange(self.robotHalfWidth + 1):
				for pnt_y in xrange(self.robotHalfLength + 1):
					# print "[Obstacle Manager] Binary image before badwaypoint:"
					# print self.mapImageBW[badwaypoint[1]+pnt_y, badwaypoint[0]+pnt_x, 0] 
					self.mapImageBW[badwaypoint[1]+pnt_y, badwaypoint[0]+pnt_x, 0] = 255
					# print self.mapImageBW[badwaypoint[1]-pnt_y, badwaypoint[0]+pnt_x, 0]
					self.mapImageBW[badwaypoint[1]-pnt_y, badwaypoint[0]+pnt_x, 0] = 255
					# print self.mapImageBW[badwaypoint[1]-pnt_y, badwaypoint[0]-pnt_x, 0] 
					self.mapImageBW[badwaypoint[1]-pnt_y, badwaypoint[0]-pnt_x, 0] = 255
					# print self.mapImageBW[badwaypoint[1]+pnt_y, badwaypoint[0]-pnt_x, 0] 
					self.mapImageBW[badwaypoint[1]+pnt_y, badwaypoint[0]-pnt_x, 0] = 255
		print "[Obstacle Manager] Initialization complete"
	...
	def get_state_validity(self, config):
		...
		mapConfig = Utils.world_to_map_s(config, self.map_info)
