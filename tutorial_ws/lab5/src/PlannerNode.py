...
class PlannerNode(object):
  ...
  def __init__(self, map_service_name, 
    ...
    if pub_topic is not None:
      self.plan_pub = rospy.Publisher(pub_topic, 
                                      PoseArray, 
                                      queue_size=1)  
  ...
  def get_plan_callback(self, req):
    ...
    if resp.success:
      resp.plan = [crdnts for sublist in self.cur_plan for crdnts in sublist]
  ...
  def update_plan(self):
    ...
    if replan:
      ...
      if self.cur_plan is not None:
        ...
        print "[Planner Node] Path length after post_processing: %f" %self.planner.get_path_length(self.cur_plan)
    ...
    if (self.cur_plan is not None) and (self.plan_pub is not None) and not self.is_plan_published:
